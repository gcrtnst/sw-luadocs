...
Teleports all vehicles in the group to the target world position.

```lua
is_success = server.setGroupPos(vehicle_id, transform_matrix)
```

Teleports all vehicles in the group to the target world position. The vehicle is displaced by other vehicles that are not in the group at the arrival point.
...
```lua
{ [i] = vehicle_id } = server.getVehicleGroup(group_id)
```
